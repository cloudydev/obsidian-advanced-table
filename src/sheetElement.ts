...
const MERGE_LEFT_SIGNIFIER = '<';
const HEADER_DELIMETER = '-';
const META_DELIMETER = '---';
...
export class SheetElement extends MarkdownRenderChild {
  ...
  parseInputToGrid() {
    ...
    return this.contentGrid = source.split(this.newLineRE)
      .map((row) => row.split(this.cellBorderRE).map(cell => cell.trim()));
  }
  ...
  normalizeGrid() {
    ...
    this.contentGrid = this.contentGrid.map((line) =>
      Array.from({ ...line, length: this.rowMaxLength }, (cell) => cell || '')
    );
  }
  ...
  getHeaderBoundaries() {
    ...
    this.headerCol = this.contentGrid[0].map((_, i) =>
      ...
      (headerCol) => headerCol.every((headerCol) => this.headerRE.test(headerCol))
    );
  }
